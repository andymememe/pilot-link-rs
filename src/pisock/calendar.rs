...
use super::blob::{
	Blob,
	BLOB_TYPE_CALENDAR_TIMEZONE_ID,
	MAX_BLOBS,
	unpack_blob
};
use super::appinfo::CategoryAppInfo;

pub const ALARM_FLAG: i32 = 64;
pub const REPEAT_FLAG: i32 = 32;
pub const NOTE_FLAG: i32 = 16;
pub const EXCEPT_FLAG: i32 = 8;
pub const DESC_FLAG: i32 = 4;
pub const LOC_FLAG: i32 = 2;
...
pub fn unpack_calendar_event(a: &mut CalendarEvent, buf: &Vec<u8>, cal_type: CalendarType) -> i32
{
	...
	if (iflags & ALARM_FLAG) != 0 {
		a.alarm = 1;
		a.advance = buf[p2_offset] as i32;
		p2_offset += 1;
		a.advance_units = buf[p2_offset] as i32;
	} else {
		a.alarm = 0;
		a.advance = 0;
		a.advance_units = 0;
	...
	if (iflags & REPEAT_FLAG) != 0 {
		...
		d = get_short(&buf[p2_offset..p2_offset+2]) as u64;
		p2_offset += 2;
		if d == 0xffff {
		...
		a.repeat_frequency = buf[p2_offset] as i32;
		p2_offset += 1;
		on = buf[p2_offset] as i64;
		p2_offset += 1;
		...
		if a.repeat_type == CalendarRepeatType::CalendarRepeatMonthlyByDay {
			a.repeat_day = num::FromPrimitive::from_i64(on).unwrap();
		} else if a.repeat_type == CalendarRepeatType::CalendarRepeatWeekly {
			for i in 0..7 {
				a.repeat_days[i] = !!((on as i32) & (1 << i));
			}
		}
		a.repeat_week_start = buf[p2_offset] as i32;
		p2_offset += 2;
	} else {
	...
	if (iflags & EXCEPT_FLAG) != 0 {
		a.exceptions = get_short(&buf[p2_offset..p2_offset+2]) as i32;
		p2_offset += 2;
	...
	if p2_offset < buf.len() {
		...
		while (buf.len() - p2_offset) > 6 {
			if blob_count >= MAX_BLOBS {
				...
				println!("Error, found more than {} blobs: {}\n", MAX_BLOBS, blob_count);
				return -1;
			}
			...
			if a.blob[blob_count].blob_type == BLOB_TYPE_CALENDAR_TIMEZONE_ID {
				...
				if result != a.blob[blob_count].length {
					println!("Read the wrong number of bytes for a timezone expected {} but was {}\n", a.blob[blob_count].length, result);
					return -1;
				}
